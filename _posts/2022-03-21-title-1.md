...
$$
h_u^{(k+1)}=UPDATE^{(k)}(h_u^{(k)},AGGREGATE^{(k)}(\left\{{h_v^{(k)},\forall v\in N(u)}\right\}))\\
...
<img src="C:\Users\BOS의스터디룸\AppData\Roaming\Typora\typora-user-images\image-20220316152831783.png" alt="image-20220316152831783" style="zoom:67%;" />
